...
SYS_FUNC(sigaction)
{
	if (entering(tcp)) {
		...
		/* act */
		print_syscall_param("act");
		decode_old_sigaction(tcp, tcp->u_arg[1]);
		tprint_arg_next();
	} else {
		/* oldact */
		print_syscall_param("oldact");
...
#if defined(SPARC) || defined(SPARC64)
		/* sa_restorer */
		print_syscall_param("sa_restorer");
