...
SYS_FUNC(getsockopt)
{
	...
	if (entering(tcp)) {
		...
		if (verbose(tcp) && tcp->u_arg[4]
			...
			/* optval */
			print_syscall_param("optval");
			printaddr(tcp->u_arg[3]);
			tprint_arg_next();

			/* optlen */
			print_syscall_param("optlen");
			printaddr(tcp->u_arg[4]);
			return RVAL_DECODED;
		}
	} else {
