		   ...
		   const struct kvm_run *run_before, const struct kvm_run *run_after)
{
	...
	printf("exit_reason=%d (%s), ready_for_interrupt_injection=%u, if_flag=%u, flags=%u, ",
	       run_after->exit_reason, str, run_after->ready_for_interrupt_injection, run_after->if_flag, run_after->flags);
	if (run_before->cr8 == run_after->cr8)
		printf("cr8=%#016llx, ", run_before->cr8);
	else
		printf("cr8=%#016llx => %#016llx, ", run_before->cr8, run_after->cr8);
	if (run_before->apic_base == run_after->apic_base)
		printf("apic_base=%#016llx", run_before->apic_base);
	else
		printf("apic_base=%#016llx => %#016llx", run_before->apic_base, run_after->apic_base);

	switch (run_after->exit_reason) {
	case KVM_EXIT_IO:
		printf(", {io={direction=%s, port=%#04x, count=%u, data_offset=%#016llx}}",
