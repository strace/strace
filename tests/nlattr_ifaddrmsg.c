...
main(void)
{
	...
	const uint8_t test_afs[] = { AF_UNSPEC, AF_INET, AF_INET6 };
	for (size_t af_idx = 0; af_idx < ARRAY_SIZE(test_afs); af_idx++) {
		uint8_t af = test_afs[af_idx];
		ifa_family = af;
		...
		for (size_t i = 0; i < ARRAY_SIZE(addr_attrs); i++) {
			if (pos < ARRAY_SIZE(address_data) &&
			    af == address_data[pos].af) {
